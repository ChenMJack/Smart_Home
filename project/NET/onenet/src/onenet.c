...
//产品id
#define PROID			"lr515XX6ao"

//设备密钥
#define ACCESS_KEY		"MXZVcFVXMVJuWFpTRXB1d1pBNzZmRHZ6ZjE0Y09CRGw="
...
//----------------------------------------------------加密-------------------------------------------------------------------------
	...
	//UsartPrintf(USART_DEBUG, "11111111\r\n");
...
_Bool OneNET_RegisterDevice(void)
{
	...
	//3.时间戳
	OneNET_Authorization("2018-10-31", PROID, 1724988635, ACCESS_KEY, NULL,
...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	switch(type)
	{
		case MQTT_PKT_PUBLISH:																//接收的Publish消息（下发命令）
		
			//再次解包（req_payloar->mqtt盒子下发数据接收到的消息体）
			...
			if(result == 0)
			{
				...
				//从cjson中提取 “params” 这个参数
				params_json = cJSON_GetObjectItem(raw_json, "params");
				//从cjson中提取 ”led“ 这个参数 ——> 后期根据需求修改
